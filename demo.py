...
from trainer import Verifier_Matcher
from trainer import Saver
...
if root=='':
    root= '/home/manashi/unit_circle_layer/ndiris/'
...
model_name=input('Filename of Model to use. (Options: DeepIris/Ours) Default: Ours ')
if model_name=='':
    model_name='Ours'
...
if cmd=='train':
    if model_name=='Ours':
        config_num=1
        model = IrisNet.IrisNetA(num_classes=N_IDENTITY, include_top=include_top) 
    else:
        config_num=2
        model = IrisNet.DeepIrisNetA(num_classes=N_IDENTITY, include_top=include_top) 
    ...
    if model_name=='Ours':
        ...
        config_num=2
        model = IrisNet.DeepIrisNetC(num_classes=N_IDENTITY, include_top=include_top)
...
if cmd=='train':
    train_img_list_file = root+train_img_list_file
    target_img_list_file = root+test_img_list_file
...
if (cmd=='test' or cmd=='save'):
    matching_list = root+matching_list
    meta_file= root+meta_file
    nonclass_id_label_dict = ut.get_id_label_map(meta_file)
    num_label_dict = ut.get_num_label_map(meta_file)
...
if cmd=='train':
    dt = cc.Casia(data_root, train_img_list_file, nonclass_id_label_dict, split='train')
    train_loader = torch.utils.data.DataLoader(dt, batch_size=Batch_Sz, shuffle=False, **kwargs)
    dv = cc.Casia(data_root, target_img_list_file, nonclass_id_label_dict, split='valid')
    val_loader = torch.utils.data.DataLoader(dv, batch_size=Batch_Sz, shuffle=False, **kwargs)
    
if cmd=='test':
    dm = cc.Casia(data_root, matching_list, nonclass_id_label_dict, split='train')
